  ...
  for iter_count in itertools.count():
    ...
    if req_type == b"READY":
      if task_queue.empty():
        ...
        router.send_multipart([req_sequence, REPLY_TYPE_NONE, b""])
        continue

      task = task_queue.get()
      router.send_multipart([req_sequence, REPLY_TYPE_TASK, json.dumps(task.dict()).encode()])
    elif req_type == b"RESULT":
      result_queue.put(Task(**json.loads(req_payload)))
      router.send_multipart([req_sequence, REPLY_TYPE_ACK, b""])
    else:
      logging.error(f"Received unrecognized request type. request: {request}, cycle: {iter_count}")
      router.send_multipart([req_sequence, REPLY_TYPE_NACK, ""])
